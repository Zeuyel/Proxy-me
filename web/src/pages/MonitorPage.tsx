...
export function MonitorPage() {
  ...
  // 加载认证文件索引映射（auth_index -> 账号名称）
  const loadAuthIndexMap = useCallback(async () => {
    try {
      const response = await authFilesApi.list().catch(() => ({ files: [] }));
      ...
      files.forEach((file) => {
        ...
        if (!map[authIndexKey]) {
          map[authIndexKey] = file.name || `auth-${authIndexKey}`;
        }
      });

      setAuthIndexMap(map);
    } catch (err) {
      console.warn('Monitor: Failed to load auth files:', err);
    }
  }, []);
