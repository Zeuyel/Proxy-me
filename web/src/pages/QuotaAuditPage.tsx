...
const normalizeRow = (value: unknown): QuotaAuditRow | null => {
  ...
  const auth = toText(firstValue(value, ['auth', 'auth_id', 'auth_index', 'account'])) || 'unknown';
...
const rowAccount = (row: QuotaAuditRow) => row.account || row.auth || 'unknown';
...
const matchesFilters = (row: QuotaAuditRow, filters: AuditFilters) => {
  if (filters.account && rowAccount(row) !== filters.account && row.auth !== filters.account) return false;
...
export function QuotaAuditPage() {
  ...
  const accounts = useMemo(() => uniqueValues(rows.map(rowAccount)), [rows]);
  ...
  const trend = useMemo(() => buildTrend(visibleRows), [visibleRows]);
  ...
  const summary = useMemo(() => {
    ...
    return {
      accounts: hasServerSummary && source?.accounts != null ? source.accounts : new Set(visibleRows.map(rowAccount)).size,
  ...
  }, [response?.summary, rows.length, visibleRows]);

  const priceSnapshot = response?.price_snapshot || visibleRows.find((row) => row.price_snapshot)?.price_snapshot;

  const quotaChartData = useMemo<ChartData<'line'>>(
    () => ({
      labels: trend.map((point) => formatTimestamp(point.timestamp, i18n.language)),
      datasets: [
        {
          label: t('quota_audit.chart_used_percent'),
        ...
        {
          label: t('quota_audit.chart_quota_delta'),
    ...
    [i18n.language, t, trend]
  );

  const costTokenChartData = useMemo<ChartData<'line'>>(
    () => ({
      labels: trend.map((point) => formatTimestamp(point.timestamp, i18n.language)),
      datasets: [
        {
          label: t('quota_audit.chart_cost_delta'),
        ...
        {
          label: t('quota_audit.chart_total_tokens'),
    ...
    <div className={styles.container}>
      ...
      {!error && response && rows.length > 0 && (
        <>
          ...
          <section className={styles.chartGrid} aria-label={t('quota_audit.trends_title')}>
            <div className={styles.chartCard}>
              ...
              <div className={styles.chartCanvas}>
                {trend.length ? <Line data={quotaChartData} options={chartOptions} /> : <p className={styles.chartEmpty}>{t('quota_audit.no_trend')}</p>}
              </div>
            </div>
            <div className={styles.chartCard}>
              <div className={styles.cardHeader}>
              ...
              <div className={styles.chartCanvas}>
                {trend.length ? <Line data={costTokenChartData} options={chartOptions} /> : <p className={styles.chartEmpty}>{t('quota_audit.no_trend')}</p>}
              </div>
            </div>
          </section>
                    ...
                    {visibleRows.map((row, index) => {
                      ...
                      return (
                        <tr key={`${row.auth}-${row.window}-${row.timestamp}-${index}`}>
                          <td>
                            <strong>{rowAccount(row)}</strong>
                            {row.auth !== rowAccount(row) && <small>{row.auth}</small>}
                          </td>
                          <td>{row.window}</td>
