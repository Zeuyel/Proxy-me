...
export function ChannelStats({
  ...
  const channelStats = useMemo(() => {
    ...
    Object.values(timeFilteredData.apis).forEach((apiData) => {
      Object.entries(apiData.models).forEach(([modelName, modelData]) => {
        modelData.details.forEach((detail) => {
          ...
          const authIndexKey = normalizeAuthIndexValue(detail.auth_index);
          const accountName = authIndexKey ? authIndexMap[authIndexKey] || `auth-${authIndexKey}` : null;

          // 获取渠道显示信息（优先使用认证文件账号）
          const { provider, masked } = getProviderDisplayParts(source, providerMap);
          const displayName = accountName ? accountName : (provider ? `${provider} (${masked})` : masked);
          const maskedKey = accountName && authIndexKey ? `#${authIndexKey}` : masked;
          const channelKey = accountName && authIndexKey ? `auth:${authIndexKey}` : `source:${source}`;
          ...
          if (!stats[channelKey]) {
            stats[channelKey] = {
              source,
              displayName,
              providerName: accountName ?? provider,
              maskedKey,
              totalRequests: 0,
              successRequests: 0,
              failedRequests: 0,
          ...
          stats[channelKey].totalRequests++;
          if (detail.failed) {
            stats[channelKey].failedRequests++;
          } else {
            stats[channelKey].successRequests++;
          }

          // 更新最近请求时间
          if (timestamp > stats[channelKey].lastRequestTime) {
            stats[channelKey].lastRequestTime = timestamp;
          }

          // 收集请求状态
          stats[channelKey].recentRequests.push({ failed: detail.failed, timestamp });

          // 模型统计
          if (!stats[channelKey].models[modelName]) {
            stats[channelKey].models[modelName] = {
              requests: 0,
              success: 0,
              failed: 0,
              successRate: 0,
          ...
          stats[channelKey].models[modelName].requests++;
          if (detail.failed) {
            stats[channelKey].models[modelName].failed++;
          } else {
            stats[channelKey].models[modelName].success++;
          }
          stats[channelKey].models[modelName].recentRequests.push({ failed: detail.failed, timestamp });
          if (timestamp > stats[channelKey].models[modelName].lastTimestamp) {
            stats[channelKey].models[modelName].lastTimestamp = timestamp;
          }
        });
      });
    });
    ...
    return Object.values(stats)
      ...
      .slice(0, 10);
  }, [timeFilteredData, providerMap, authIndexMap]);
