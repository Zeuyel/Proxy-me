      ...
      Object.values(apiData.models).forEach((modelData) => {
        modelData.details.forEach((detail) => {
          if (detail.failed) {
            ...
            failedSources.add(authIndexKey ? `auth:${authIndexKey}` : `source:${source}`);
          }
        });
      });
    });
    ...
    Object.values(timeFilteredData.apis).forEach((apiData) => {
      Object.entries(apiData.models).forEach(([modelName, modelData]) => {
        modelData.details.forEach((detail) => {
          ...
          // 只统计有失败记录的渠道
          if (!failedSources.has(channelKey)) return;
          ...
          if (detail.failed) {
            stats[channelKey].failedCount++;
            if (timestamp > stats[channelKey].lastFailTime) {
              stats[channelKey].lastFailTime = timestamp;
            }
          }
          ...
          stats[channelKey].models[modelName].total++;
          if (detail.failed) {
            stats[channelKey].models[modelName].failure++;
          } else {
